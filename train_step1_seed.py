...
class DulClsTrainer(mlib.Faster1v1):
    ...
    def _model_loader(self):
        ...
        self.model['AU_branch']  = mlib.AU_branch(self.args) #dawn
        self.model['criterion'] = mlib.ClsLoss(self.args, branch='AU') # dawn define the loss function
        self.model['optimizer'] = torch.optim.SGD(
                                    [{'params': self.model['backbone'].parameters()},
                                    {'params': self.model['AU_branch'].parameters()}],
        ...
        if self.use_gpu:
            ...
            self.model['AU_branch']  = self.model['AU_branch'].cuda()
        ...
        if self.use_gpu and len(self.args.gpu_ids) > 1:
            ...
            self.model['AU_branch'] = torch.nn.DataParallel(self.model['AU_branch'], device_ids=self.args.gpu_ids)
        ...
        if len(self.args.resume) > 2:
            ...
            self.model['AU_branch'].load_state_dict(checkpoint['AU_branch'])
    ...
    def _train_one_epoch(self, epoch = 0, sampling_times=3, num_classes=10):

        self.model['backbone'].train()
        self.model['AU_branch'].train()
        predictions = []
        ...
        for idx, (img, gty) in enumerate(self.data['train']):
            ...
            for _ in range(sampling_times):
                mu, logvar, output = self.model['AU_branch'](embedding)
                loss = self.model['criterion'](output, gty, epoch, mu, logvar)
    ...
    def _test_one_epoch(self):
        ...
        with torch.no_grad():
            for idx, (img, gty) in enumerate(self.data['test']):
                ...
                _,_,output  = self.model['AU_branch'](embedding)
                predy   = np.argmax(output.data.cpu().numpy(), axis=1)  # TODO
    ...
    def _save_model(self):
        ...
        save_name = '%s/AU_backbone_%s_epoch_%02d.pth' % \
        ...
        torch.save({
            ...
            'AU_branch': self.model['AU_branch'].state_dict(),}, save_name)
    ...
    def _dul_training(self):
        ...
        for epoch in range(self.args.start_epoch, self.args.end_epoch + 1):
            ...
            train_loss = self._train_one_epoch(epoch, sampling_times=1)
