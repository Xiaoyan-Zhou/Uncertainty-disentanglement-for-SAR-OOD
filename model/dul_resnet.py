...
class DULResNet(nn.Module):
    def __init__(self, block, layers, feat_dim = 512, drop_ratio = 0.4, use_se = True, used_as = 'backbone', use_rayleigh=False):
        '''
        ...
        self.layer0_cond = nn.Sequential(
            nn.Conv2d(4, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(self.inplanes),
            nn.PReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2))
    ...
    def _reparameterize_rayleigh(self, mu, logvar):
        # 原本的标准差 std 从 logvar 计算得来
        std = torch.exp(0.5 * logvar)
        
        # 从 [0, 1] 的均匀分布中采样
        U = torch.rand_like(std)
        
        # Rayleigh分布的采样公式: r = sigma * sqrt(-2 * log(U))
        rayleigh_sample = std * torch.sqrt(-2 * torch.log(U))
        
        # 将Rayleigh分布样本加到均值mu上
        return mu + rayleigh_sample


    def forward(self, x):
        if x.shape[1] == 4: # +uncertainty channel
            x = self.layer0_cond(x)
        else:           
            x = self.layer0(x)
        x = self.layer1(x)
        x = self.layer2(x)
        ...
        if self.used_as == 'backbone':
            ...
            if self.use_rayleigh:
                embedding = self._reparameterize_rayleigh(mu, logvar)
            else:
                embedding = self._reparameterize(mu, logvar)
        return (mu, logvar, embedding) # train_uncertainty.py
        
    
def dulres_zoo(backbone = 'dulres18', feat_dim = 512, drop_ratio = 0.4, \
               use_se = True, used_as = 'baseline', use_rayleigh=False):
    zoo_dict = {
        'dulres18' : [2, 2, 2, 2],
        'dulres50' : [3, 4, 6, 3],
        'dulres101': [3, 4, 23,3],
    ...
    return DULResNet(block, zoo_dict[backbone], feat_dim, drop_ratio, use_se, used_as, use_rayleigh)
