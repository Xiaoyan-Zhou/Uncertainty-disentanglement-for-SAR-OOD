...
class DULResNet(nn.Module):
    def __init__(self, block, layers, feat_dim = 512, drop_ratio = 0.4, use_se = True, used_as = 'baseline'):
        '''
        use_as = baseline : just use mu_head for deterministic model
        use_as = dul_cls  : use both mu_head and logvar_head for dul_cls model
        use_as = backbone : neither mu_head nor logvar_head is used, just for feature extracting.
        '''
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        if self.use_as == 'backbone':
            mu = x
            logvar = None, 
            embedding = None
        elif self.use_as == 'baseline':
            mu = self.mu_head(x)
            logvar = None, 
            embedding = None
        else:
